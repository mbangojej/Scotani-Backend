...
exports.list = async (req, res, next) => {
    try {
        ...
        if (query != undefined && query != '') {
            filter = {
                $or: [
                    { firstName: new RegExp(query.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'), 'gi') },
        ...
        var docs = await User.aggregate([
            ...
            {
                $project: {
                    firstName: 1, email: 1, image:1 , accountStatus: 1, phone: 1, xOauth: 1, confirmationCode: 1,
                    // role
                    'role._id': '$role._id',
                    'role.title': 1,
                    // branch
        ...
        return res.status(200).send({ success: true, data: { docs, total, page, limit, pages } });
    } catch (error) {
        next(error);
    }
}
